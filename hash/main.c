...
unsigned int hashfunc_str(unsigned int buckets, void *key)
{
  char *sno = (char*)key;
...
unsigned int hashfunc_int(unsigned int buckets, void *key)
{
  int *sno = (int*)key;
  return (*sno) % buckets;
}
...
int main()
{
  ...
  // hash_t *hash = hash_alloc(256, hashfunc_str);

  // int size = sizeof(stu_arr)/sizeof(stu_t);
  ...
  hash_t *hash = hash_alloc(256, hashfunc_int);
