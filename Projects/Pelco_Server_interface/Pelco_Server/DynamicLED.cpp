...
CDynamicLED::CDynamicLED()
{
	...
	m_bOn = FALSE;
	m_nID = ID_LED_GREEN; //ID_LED_RED;
	m_nShape = ID_SHAPE_ROUND;//ID_SHAPE_SQUARE;//
	m_nBrightness = 255;
}
...
void CDynamicLED::SetLEDBrightness(int brightness)
{
...
int CDynamicLED::GetLEDBrightness()
{
...
// 	CClientDC dc(this);
	...
	if(m_nID==ID_LED_RED)
	{
		...
		m_PenBright.CreatePen(0,1,RGB(m_nBrightness,0,0));
		m_BrushBright.CreateSolidBrush(RGB(m_nBrightness,0,0));

		// Here i'm creating a dark shade of red. You can play with the values to
		// see the effect on the LED control

		m_PenDark.CreatePen(0,1,RGB(100,0,0));
		m_BrushDark.CreateSolidBrush(RGB(170,170,170));
	}
	else if(m_nID==ID_LED_GREEN)
	{
		// If the user has selected GREEN as the color of the LED

		m_PenBright.CreatePen(0,1,RGB(0,m_nBrightness,0));
		m_BrushBright.CreateSolidBrush(RGB(0,m_nBrightness,0));
	...
	else if(m_nID==ID_LED_BLUE)
	{
		...
		m_PenBright.CreatePen(0,1,RGB(0,0,m_nBrightness));
		m_BrushBright.CreateSolidBrush(RGB(0,0,m_nBrightness));
	...
	else if(m_nID==ID_LED_YELLOW)
	{
		...
		m_PenBright.CreatePen(0,1,RGB(m_nBrightness,m_nBrightness ,0));
		m_BrushBright.CreateSolidBrush(RGB(m_nBrightness,m_nBrightness,0));
...
void CDynamicLED::OnTimer(UINT_PTR nIDEvent) 
{
